...
public class MNIST {
    ...
    public static class DataHolder implements Comparable<DataHolder> {
         ...
         * @throws IllegalArgumentException thrown when the image lengths are not equal.
        ...
        public static float totalDist(float[] img1, float[] img2) throws IllegalArgumentException {
            ...
            for (int i = 0; i < img1.length; i++) {
                float difference = img1[i] - img2[i];
                float squaredDifference = (float) Math.pow(difference, 2);
        ...
        public static DataHolder[] getClosestMatches(float[] image, int k) {
            PriorityQueue<DataHolder> pQueue = new PriorityQueue<>(TRAIN_IMAGES.length);
            DataHolder[] kNeighbors = new DataHolder[k];
